...
func start(text: String):
	...
	tween.tween_property(self, "scale", Vector2.ONE, .3).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_ELASTIC)
	tween.chain()
	...
	tween.tween_property(self, "scale", Vector2.ONE, .4).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_CUBIC)
	tween.chain()
